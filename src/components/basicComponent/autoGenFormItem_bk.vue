...
<!--    <div>
    ...
    <div >
      <Form label-position="left" class=""
            :model="allValue" :label-width="autoGenFormItemInfo.labelWidth"
            :ref="autoGenFormItemInfo.refName" inline @submit.native.prevent
      >
        <template  v-for="(ele,idx) in allValue.inputValue" >
          <!--<div>-->
            <!--<Col span="24">-->
            <FormItem
              :prop="'inputValue.'+idx" :key="idx"
            ...
            >
              ...
              <Input
                ...
                v-model="allValue.inputValue[idx]" :placeholder="inputAttribute[idx]['placeHolder'][0]"
...
<script>
  ...
  export default {
      ...
      methods: {
        ...
        disableEditable(){
          let tobeDeletedIdx=searchEmptyValueIdxInArray({array:this.allValue.inputValue})
          // inf('tobeDeletedIdx',tobeDeletedIdx)
          if(tobeDeletedIdx.length>0){
            deleteDefinedEleInArray({array:this.allValue.inputValue,tobeDeletedIdx:tobeDeletedIdx})
        ...
        addItem(){
          if(this.allValue.inputValue.length<this.autoGenFormItemInfo.maxNum){
            this.allValue.inputValue.push('')
        ...
        removeItem({idx}){
          this.allValue.inputValue.splice(idx,1)
        ...
        focusInputPlaceHolderDisappear({idx}){
          if (undefined === this.allValue.inputValue[idx] || null === this.allValue.inputValue[idx] || '' === this.allValue.inputValue[idx]) {
...
/*          this.$refs[this.autoGenFormItemInfo.refName].validateField(idx, (validResult) => {
        ...
        validateDuplicate({idx}){
          ...
          let tobeCheckValue=this.allValue.inputValue[idx]
          if(''!==tobeCheckValue){
            // this.inputTempData[idx][InputTempDataFieldName.VALID_RESULT] =null
            for(let index in this.allValue.inputValue){
              // inf('index',index)
              //skip self
              if(idx!==parseInt(index)){
                let otherValue=this.allValue.inputValue[index]
...
/*          inf('flag',flag)
      ...
      data() {
          ...
          return {
            allValue:{inputValue:this.autoGenFormItemInfo.inputValue},//无需从props传入
            inputValue:this.autoGenFormItemInfo.inputValue,
